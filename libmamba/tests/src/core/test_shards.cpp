...
TEST_CASE("Shards - python minor prefilter")
{
    ...
    auto run_for_dep = [&](const std::string& dep,
                           std::optional<std::string> python_minor) -> expected_t<ShardDict>
    {
    ...
    {
        auto result = run_for_dep("python >=3.11,<3.12", "3.12");
        REQUIRE(result.has_value());
    ...
    {
        auto result = run_for_dep("python >=3.12,<3.13", "3.12");
        REQUIRE(result.has_value());
        REQUIRE(result->packages.size() == 1);
        REQUIRE(result->packages.begin()->second.name == "test-pkg");
    }

    SECTION("no python minor context means no filtering")
    {
